...
#define STATIC_DEBUGPRINT(fp, expr, msgs...)\
	do{\
		if (expr) fprintf(fp, msgs);\
	}while(0)

#define DEBUGF(level, msgs...)\
	do{\
		STATIC_DEBUGPRINT(stderr, DEBUG_EXPR_LEVEL(level), msgs);\
	}while(0)

#define DEBUGTOPF(topic, msgs...)\
	do{\
		STATIC_DEBUGPRINT(stderr, DEBUG_EXPR_TOPIC(topic), msgs);\
	}while(0)
...
// measurement chain
#ifdef	MEASURE_WTIME
#define MEASURE_LENGTH 128
#define	MWTIME_PRINT(fh_p, msgs...) \
	do{\
		struct timeval tv;\
		char deblocfbuf[MEASURE_LENGTH];\
		gettimeofday(&tv, NULL);\
		sprintf(deblocfbuf, msgs);\
		fprintf(fh_p, "MT[%d][%f]%s", getpid(), tv2dbl(&tv), deblocfbuf);\
	}while(0)
#else
#define MWTIME_PRINT(fh_p, msgs...)
#endif
/*
#define MTIME_LOC(fh_p, msgs...)\
	do{\
		time_t timet;\
		struct tm *tm;\
		char deblocfbuf[DEBUG_LENGTH];\
		...
		time(&timet);\
		tm = localtime(&timet);\
		fprintf(fh_p, level, "%2d/%02d %2d:%02d:%02d[%s,%d]:%s\n",\
				tm->tm_mon + 1, tm->tm_mday,\
				tm->tm_hour, tm->tm_min, tm->tm_sec, __FILE__, __LINE__, deblocfbuf);\
	}while(0)
*/
